          ...
          <div className="flex flex-col gap-4 w-full md:w-1/2">
            ...
            {isAdmin ? (
              <>
                <label>
                  Descrição:
                  <input type="text" value={carro.descricao} onChange={e => handleCampoChange('descricao', e.target.value)} className="w-full border rounded px-2 py-1 mt-1" />
                </label>
                <label>
                  Preço:
                  <input type="number" value={carro.preco} onChange={e => handleCampoChange('preco', e.target.value)} className="w-32 border rounded px-2 py-1 mt-1" />
                </label>
                <label>
                  Estoque:
                  <input type="number" value={carro.estoque} onChange={e => handleCampoChange('estoque', e.target.value)} className="w-24 border rounded px-2 py-1 mt-1" />
                </label>
              </>
            ) : (
              <>
            ...
            <div className="mt-6 flex gap-4">
              {!isAdmin && (
                <button onClick={handleAddToCart} disabled={carro.estoque === 0} className={`${carro.estoque === 0 ? 'bg-gray-400 cursor-not-allowed' : 'bg-[#3e2f2f] hover:bg-[#5e3a1f]'} px-6 py-2 rounded-md text-white transition`}>
                  Adicionar ao carrinho
                </button>
              )}
              <button onClick={() => navigate(-1)} className="px-4 py-2 text-sm border border-[#5e3a1f] text-[#5e3a1f] rounded hover:bg-[#f3e6d8]">Voltar</button>
            </div>
